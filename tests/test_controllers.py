...
from controllers.base import BaseController, ControllerConfig, ControllerMetrics, StimState
...
class DummyHDCController(HDCController):
    ...
    def __init__(self, score: float = 1.0, config: ControllerConfig | None = None) -> None:
...
def _make_lfp_chunk(n_samples: int, value: float = 0.0) -> tuple[np.ndarray, np.ndarray]:
...
class TestStateMachine:
    ...
    def _make_triggering_controller(
    ...
    def test_stim_transitions_to_lockout_after_burst_duration(self):
        ...
        stim_duration_samples = int(
            ctrl.config.stim_duration_ms * ctrl.config.fs / 1000.0
        ) + 5  # a few extra to be sure the state machine fires
...
class TestMetrics:
    ...
    def test_to_dict_has_expected_keys(self):
        ...
        for key in ("n_decisions", "n_detections", "n_stimulations",
                    "duty_cycle", "total_stim_samples", "total_samples",
                    "stim_times_ms"):
...
class TestRunClosedLoop:
    ...
    def test_stim_array_nonzero_when_stimulating(self, short_patho_config):
        """Where state == STIMULATING, stim must equal stim_amplitude."""
        from controllers.run_controller import run_closed_loop
        ...
        if is_stim.any():
            np.testing.assert_allclose(stim[is_stim], 1.5)
